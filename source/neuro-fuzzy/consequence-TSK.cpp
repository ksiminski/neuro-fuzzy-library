...
std::ostream& ksi::consequence_TSK::prettyPrint(std::ostream& ss, const DescriptorStatistics& descStat)
{
    for (std::size_t i = 0; i < _params.size() - 1; ++i) {
        ss << i+1 << ". slope has " << (_params[i] <= descStat.std_dev ? "low " : "high ") << (_params[i] < 0 ? "negative " : "positive ") << "importance AND ";
    }
